...
if df_raw is not None and not df_raw.empty and specialty_dfs:
    ...
    for _sn_p, (_sraw_p, _, _) in specialty_dfs.items():
        ...
        if not _col_ped or _col_ped not in _sraw_p.columns:
            continue
        # Recorrer GUIA 1, GUIA 2, GUIA 3 — cualquiera puede mapear a un pedido
        for _gname in ("GUIA 1", "GUIA 2", "GUIA 3"):
            ...
            if not _col_g or _col_g not in _sraw_p.columns:
                continue
            _sub = _sraw_p[[_col_g, _col_ped]].dropna(subset=[_col_g])
    ...
    if _col_guia_main and _col_guia_main in df_raw.columns:
        df_raw = df_raw.copy()
        df_raw["_n_pedido"] = (
            df_raw[_col_guia_main].astype(str).str.strip()
            .map(_pedido_lookup)
            .fillna("NA")
        )
